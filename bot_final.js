...
function createQueueEmbed(queue, gameMode) {
    ...
    // Título exato do realtrem
    const isFull = filledSlots >= maxPlayers;
    const status = isFull ? 'Partida Iniciada!' : 'Fila Normal';
    const title = `${gameMode} | ${status}`;
    
    // Descrição idêntica ao realtrem
    const description = `Seja Bem Vindo(a) a fila **Normal**! Aqui todos os times são formados aleatoriamente. Caso deseje participar, utilize os botões abaixo para fazer as ações disponíveis.`;
    ...
    const embed = new EmbedBuilder()
        ...
        .addFields(fields)
        .setThumbnail(client.user.displayAvatarURL({ format: 'gif', size: 512 })); // STAR BOT animado
    ...
    switch (action) {
        case 'enter':
            ...
            if (queue.includes(userId)) {
                return interaction.reply({ content: '⚠️ **VOCÊ JÁ ESTÁ NA FILA!** ⚠️\n\n🛡️ Você já se inscreveu nesta fila\n🔥 Aguarde a equipe completar!\n⚔️ **Prepare-se para jogar!**', ephemeral: true });
            }
        ...
        case 'leave':
            ...
            // Resposta simples para sair
            await interaction.reply({ content: `👋 **SAIU DA FILA!** 👋\n\n🛡️ Você foi removido da equipe\n🔄 **Você pode entrar novamente quando quiser!**`, ephemeral: true });
            break;
