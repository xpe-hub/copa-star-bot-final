...
class QueueView(View):
    def __init__(self, game_mode):
        ...
        queue = queues.get(game_mode, {'players': [], 'teams': [[], []]})
...
def is_user_in_any_queue(user_id):
    for game_mode, queue in queues.items():
        if any(p['id'] == str(user_id) for p in queue['players']):
            return game_mode
    return None

# Función para crear embed de la fila
def create_queue_embed(game_mode, is_closed=False):
    queue = queues.get(game_mode, {'players': [], 'teams': [[], []]})
...
async def handle_queue_action(interaction, action):
    ...
    try:
        ...
        if action == 'join':
            ...
            if queues.get(f'{game_mode}_closed'):
                await interaction.response.send_message(
...
async def update_queue_message(channel, game_mode):
    try:
        message_id = queue_messages.get(game_mode)
        if message_id:
            message = await channel.fetch_message(message_id)
            is_closed = queues.get(f'{game_mode}_closed', False)
            
            await message.edit(
                embed=create_queue_embed(game_mode, is_closed),
                view=QueueView(game_mode)
            )
    except Exception as error:
...
async def on_message(message):
    ...
    if command == 'fila':
        game_mode = args[1].lower() if len(args) > 1 else '1v1'
        
        if game_mode not in ['1v1', '2v2']:
            await message.reply('❌ **¡Modo inválido!**\n\nUsa: `!fila 1v1` o `!fila 2v2`')
            return
...
async def handle_message_commands(message):
    ...
    if command == 'fila':
        ...
        # Verificar se já existe uma fila ativa
        if game_mode in queues and not queues.get(f'{game_mode}_closed'):
            await message.reply(
                'ℹ️ **¡Ya existe una fila activa!**\n\nUsa los botones en el mensaje de la fila para participar.'
            )
